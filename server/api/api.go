...
// This package is for serving the actual chargerwallet API.
...
func corsValidator() (OriginValidator, error) {
	...
	okregex, err := regexp.Compile(`^https://([[:alnum:]\-_]+\.)*chargerwallet\.so$`)
	if err != nil {
		return nil, err
	}

	okcnregex, err := regexp.Compile(`^https://([[:alnum:]\-_]+\.)*chargerwalletcn\.com$`)
	if err != nil {
		return nil, err
	}
