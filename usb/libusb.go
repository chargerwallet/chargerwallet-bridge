...
import (
	...
	lowlevel "github.com/chargerwallet/chargerwallet-bridge/usb/lowlevel/libusb"
...
If you run chargerwallet in an environment without USB (for example, docker or travis), use '-u=false'. For example, './chargerwallet-go -e 21324 -u=false'.
...
func matchType(dd *lowlevel.Device_Descriptor) core.DeviceType {
	...
	if dd.IdProduct == core.ChargerWalletBootloader {
		return core.TypeT2Boot
	}
...
func (b *LibUSB) matchVidPid(vid uint16, pid uint16) bool {
	...
	chargerwallet := vid == core.VendorChargerWallet && (pid == core.ChargerWalletirmware || pid == core.ChargerWalletBootloader)

	if b.only {
		trezor1 := vid == core.VendorT1 && (pid == core.ProductT1Firmware)
		return trezor1 || trezor2 || chargerwallet
	}

	return trezor2 || chargerwallet
}
